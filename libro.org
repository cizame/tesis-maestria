...
Es por eso que el tener algoritmos de ordenamoento eficientes es
importante pues como Knuth hace referencia gran parte del tiempo
gastado se debe problablemente al uso de algoritmos ineficientes.

Si se desea ordenar una lista de \(n\) elementos de mayor a menor, una
forma de hacerlo es enconontrar el mayor de los \(n\) lo que se hace
en \(n-1\) pasos, y colocarlo al principio de la lista, despues
encontrar el mayor de los \(n-1\) en \(n-2\) pasos y ponerlo en el
segundo lugar y así susesibamente hasta los \(n-(n-2)=2\) en un solo
paso y colocarlos al final. Por lo tanto el tiempo en el que se ordena una lista usando este
algoritmo es: 
\begin{align*}
...
Existen diversos algoritmos de ordenamiento unos más famos que otros,
aún que eso no signifaca que sean los más eficientes.
